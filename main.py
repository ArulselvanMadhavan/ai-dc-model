...
llama405b_cluster = ClusterSpecs(TP=8*16, DP=128, scale_up=900*GIGA,scale_out=100*GIGA, HB=8)

opt175b = ModelSpecs(1000, 2048, 12288, 4*12288, 50272, 96, True, False, None, Dtypes.FP16, 1, 1, False, "opt175b", 140000)
...
llama405b_text = ModelSpecs(16*128, 8192, 16384, 3.25*16384, 128256, 126, True, False, None, Dtypes.FP16, 8, 128, True, "llama405B", 930000)
...
if __name__ == "__main__":
    tspecs = [opt175b_prod, llama405b_prod]
    for tspec in tspecs:
            ...
            for d in out_data:
                ...
                if isinstance(evt, simpy.events.Timeout) and evt.value is not None:
                    if isinstance(evt.value, EventData) and "ckpt" in "_".join(evt.value.name):
                        ...
                        break
            print(model_specs.name, xpu_specs.name, cluster_specs.TP,
                  cluster_specs.DP, cluster_specs.HB, iter_time, (iter_time * model_specs.num_iters) / (MICRO * 60*60*24))
            reset_cid()
